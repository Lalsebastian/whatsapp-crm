...
async function handleButton(from, id, session) {
  ...
  // Ladies products (same products, back goes to ladies_products)
  ...
  if (id === 'ladies_appointments') return showLadiesAppointments(from, session);
...
async function showGentsAppointmentServices(from, session) {
  ...
  await sendListMessage(from, 'Please select the service you want to book:', 'Choose service',
    [{ title: 'Gents Services', rows: [
...
async function saveApptService(from, session, service) {
  ...
  await sendListMessage(from, 'Please select your preferred branch:', 'Choose branch',
...
async function showGentsOffers(from) {
  ...
  await sendButtons(from, `Tap below to book any package:`,
    [{ id: 'gents_appointments', title: 'Book now' }, { id: 'gents_main', title: 'Back to menu' }]
  );
}

// ═══════════════════════════════════════════════════════
// SHARED PRODUCTS (used by both gents & ladies)
// ═══════════════════════════════════════════════════════
async function showProdHairCare(from, backId) {
  ...
  await sendButtons(from, `For orders or enquiries contact us directly.`, [{ id: backId, title: 'Back to products' }]);
}
...
async function showLadiesWaxing(from) {
  ...
  await sendButtons(from, `Ready to book?`,
    [{ id: 'ladies_appointments', title: 'Book appointment' }, { id: 'ladies_services', title: 'Back' }]
  );
}
...
async function showLadiesLocations(from) {
  await sendButtons(from,
    `📍 *Ladies Saloon Locations:*\n\nSelect your branch to view address and map:`,
    [
      { id: 'l_loc_muteena', title: 'Muteena' },
      { id: 'l_loc_dip', title: 'DIP-2' },
    ]
  );
}
...
async function showTimeSlots(from) {
  ...
  await sendButtons(from, `More slots:`, [{ id: 'time4', title: '12-01 PM' }]);
}
